...
namespace Base.Infra.Repositorios.Usuarios
{
    ...
    {    
        ...
        public async Task<Retorno> Logar(LoginDTO login)
        {
            ...
            {                
                ...
                var userToken = new TokenUsuario { Id = user.Id, Usuario = user.UserName, Email = user.Email };
        ...
        public async Task<UsuarioLogadoDTO> GerarJwtAsync(UsuarioLogadoDTO login)
        {
            try
            {
                //var permissoes = await _ctx.ViewPerfilUsuario.AsNoTracking().Where(a => a.IdPerfil == login.UserToken.IdPerfil).ToListAsync();
        ...
        private async Task<Usuario> GetUsuarioById(string id)
        {
            try
            {
